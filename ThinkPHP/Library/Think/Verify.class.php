    ...
    public function entry($id = '') {
        ...
        if($this->useZh){ // 中文验证码
            for ($i = 0; $i<$this->length; $i++) {
                ...
                imagettftext($this->_image, $this->fontSize, mt_rand(-30, 20), $this->fontSize*($i+1)*1.5, $this->fontSize + mt_rand(10, 20), $this->_color, $this->fontttf, $code[$i]);
            }
        }else{
            for ($i = 0; $i<$this->length; $i++) {
                $code[$i] = $this->codeSet[mt_rand(0, strlen($this->codeSet)-1)];
                $codeNX  += mt_rand($this->fontSize*1, $this->fontSize*1.2);
                imagettftext($this->_image, $this->fontSize, mt_rand(-30,20), $codeNX, $this->fontSize*1.6, $this->_color, $this->fontttf, $code[$i]);
            }
        }
